...
// уже нет.
//
// Но случайность бывает не у всякого вида места, и потому потеря — с ручкой.
// Пустой ягодник — неудача: ягоды отрастут, и приходить сюда снова есть
// смысл. Достроенная площадка — не неудача, а ЗНАНИЕ: работы здесь больше
// нет и не будет, и второй раз это не выяснится иначе.
//
// Разница стоила забора. Место работы гоблин помечает каждый тик, пока на
// нём работает (иначе он не вернулся бы к начатому), — и к концу стройки
// оно лежит в голове на полной твёрдости. Достроив навес, он уходил за
// материалом, вспоминал самое твёрдое место работы, шёл к нему через полкарты,
// не находил работы, терял четверть — и повторял это ЧЕТЫРЕЖДЫ, прежде чем
// голова его отпускала. Замер: из 4671 похода за материалом по памяти на
// увиденную рядом площадку приходилось 80. Кол, воткнутый в двух шагах от
// собственной лежанки, не получал работы вовсе.
inline void disappoint(KnowledgeComponent& mind, PlaceKind kind, int x, int y, int loss = kDisappointLoss) {
    for (auto& place : mind.places) {
        if (place.kind == kind && place.x == x && place.y == y) {
            place.strength -= loss;
            if (place.strength <= 0) {
