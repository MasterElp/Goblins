...
// Число идёт В НОГУ с темпом жизни (core/Scale.hpp) и поделено под нынешний
...
// и здесь.
//
// Разойдись они — охота потеряла бы смысл: хищник ест вдесятеро медленнее, и
// туша сгнивала бы прежде, чем он её доест.
constexpr int kCarcassRot = 2;
