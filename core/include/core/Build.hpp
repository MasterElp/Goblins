...
// Раз во сколько тиков постройка теряет единицу прочности. НОЛЬ — не ветшает
// вовсе, и сейчас в мире стоит именно ноль.
//
// Выключено намеренно и до поры. Пока не видно, доводится ли задуманное до
// конца, поддержание смотреть мешает: оно съедает труд в том же месте, где
// его не хватает, и по итогу нельзя отличить "не строят" от "строят, да
// осыпается". Замер, ради которого это и снято: число клеток с плетнём росло
// до четырёхтысячного тика и дальше ПАДАЛО, 259 -> 119, — плетень ветшал
// быстрее, чем его доводили.
//
// Прежде здесь стояло 400, и число было подобрано замером, а не на глаз: при
// четырёх тиках поддержание навеса стоило около сотни работ на тысячу тиков —
// четверть всего времени гоблина, — и лагерь жил в вечном ремонте: двадцать
// начатых строек на пятнадцать живых, средняя прочность навеса 59 из ста.
// Четыреста давало десять работ на тысячу тиков: заметно, но не вместо жизни.
// Число это стоит вернуть здесь же, когда стройка начнёт доходить до конца.
//
// Что становится мёртвым, пока ноль, — сказать прямо: путь "осыпавшееся до
// нуля исчезает совсем" (снятие BuildingComponent) больше не проходится
// никогда, и постройка, однажды доведённая, стоит вечно.
//
// Ветшание было и остаётся ОТ ВРЕМЕНИ и только от времени — не от того, кто
// на постройке спал: наказание за людность превратило бы большой лагерь в
// самый непрочный, а он должен быть самым крепким, просто потому, что в нём
// больше рук. И когда срок вернётся, он обязан идти В НОГУ с темпом жизни
// (core/Scale.hpp): у этого срока нет породы, значит нет и множителя, которым
// его делить. Меняешь темп — меняй и здесь.
constexpr int kBuildDecayPeriod = 0;
...
//
// Проверка на ноль стоит ПЕРВОЙ, а не подставляется числом: срок — делитель,
// и ноль здесь не "очень редко", а деление на ноль.
inline bool buildDecays(std::uint64_t tick, std::size_t cell) {
    if (kBuildDecayPeriod <= 0) {
        return false;
    }
