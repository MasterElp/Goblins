...
// Во сколько раз больше крупиц тело способно держать сверх того, что нужно
// ему самому. Вдвое — то есть половина носимого вещества всегда лишняя для
// собственного роста и годится на потомство.
//
// Без запаса роды были почти невозможны, и вот почему. Потолок стоял ровно
// на собственной нужде, а родить можно, лишь накопив её целиком (см.
// AnimalSystem, canBearYoung) — значит условие выполнялось короткими окнами
// сразу после сытной еды и тут же рушилось: пищеварение уносит крупицу раз
// в kDungPeriod тиков. Хищник, который ест раз в триста тиков, жил вечно на
// единицу ниже порога. Замер: готовых плодиться оказалось трое из
// семнадцати хищников, двадцать из четырёхсот травоядных и трое из
// семидесяти пяти гоблинов.
//
// С запасом сытый живёт выше порога постоянно: утечка в навоз съедает
// излишек, а не последнюю крупицу перед порогом. Мать при этом не беднеет —
// отдав детёнышу его полную нужду, она опускается со вдвойне до ровно
// своей, а потолок роста считается от своей нужды.
constexpr int kProteinReserve = 2000;
...
inline void advanceBody(AnimalComponent& state, const AnimalGenomeComponent& genome, int pace,
    ...
    // Мёртвое не заживает. Условие на здоровье стоит рядом с сытостью, а не
    // где-то выше, потому что беда ровно здесь: тело с нулём здоровья ещё не
    // похоронено — хоронит его система, и не раньше, чем спросит bodyDied.
    // Между ударом и вопросом успевает пройти заживление, и добитый ровно в
    // ноль вставал бы с двойкой здоровья. Пока зубы доставались одному
    // зверью, это было почти незаметно: AnimalSystem хоронит убитого в том же
    // проходе, где бьёт. У гоблина бьёт одна система, а хоронит другая, и
    // промежуток стал настоящим.
    if (state.energy > 0 && state.water > 0 && state.health > 0) {
...
inline void takeProtein(AnimalComponent& state, const AnimalGenomeComponent& genome, int grains) {
    const int cap = std::max(1, proteinNeedOf(genome) * kProteinReserve / kFull);
