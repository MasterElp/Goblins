...
struct GenerationStats {
    ...
    // Сколько всего клеток помечено источником (WaterSourceComponent): по
    // одной на реку плюс родники. Ровно эта цифра и решает, зальёт ли мир
    // водой — каждый источник стоит постоянным столбом и отдаёт примерно
    // свою глубину за тик, — поэтому её видно в консоли сразу.
    int waterSourcesPlaced = 0;
