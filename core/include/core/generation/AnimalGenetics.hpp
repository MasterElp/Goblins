...
inline constexpr AnimalTrait kHerbivoreTraits[] = {
    ...
    // Рога: чем их больше, тем дольше хромает укусивший хищник (см. п.8
    // "Зубы" в AnimalSystem). Черта есть только у травоядного — на хищника
    // не охотятся, и отдача ему не нужна: та же асимметрия, что у attack в
    // хищничьей таблице.
    //
    // Куплены рога тем же бюджетом, что и всё остальное, и в этом весь их
    // смысл: вложенное в рога не вложено в скорость. Получаются два разных
    // способа не быть съеденным — убежать и не дать себя догнать второй
    // раз, — а не одно бесплатное улучшение вида.
    {"defense", &AnimalGenomeComponent::defense, 0, 1000, 1.0f},
    // Меткость тех же рогов: как часто удар вообще попадает (розыгрыш в
    // AnimalSystem, п.8 "Зубы"). Раньше это было одно число на весь мир —
    // теперь вид может вложиться в частые слабые уколы или редкие, но
    // верные, вместо того чтобы у всех рога попадали одинаково.
    //
    // Нижняя граница не ноль: рога, которые не попадают никогда, — это уже
    // выражено тем, что defense == 0 отключает розыгрыш целиком, а не
    // отдельной меткостью впустую. Дешевле defense: сама по себе меткость
    // без силы удара защищает слабее.
    {"gore_chance", &AnimalGenomeComponent::goreChance, 200, 950, 0.75f},
};
