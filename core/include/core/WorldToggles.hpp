...
struct WorldToggles {
    ...
    // Оставляют ли ноги след на земле и тянет ли этот след к себе идущего
    // (core/Trample.hpp, core/Walk.hpp). Гасит и то, и другое разом, и
    // заодно зарастание: тропы, набитые до выключения, иначе продолжали бы
    // таять.
    //
    // Выключатель заведён не для красоты панели, а ради замера. Мера троп —
    // это ПАРА прогонов, и делать её двумя разными сборками значит
    // сравнивать заодно всё, что успело измениться в ядре между ними. Один
    // бинарник, два прогона, разница в одном поле.
    bool trampling = true;

    // Каким разумом думают существа: выключено — жадным (наибольший вес),
    // включено — жребием по весам (core/Mind.hpp).
    //
    // Переключателем, а не сборкой, и ровно по той же причине, по какой
    // переключается вытаптывание: сравнивать два разума надо ОДНИМ
    // бинарником на одном seed. Двумя сборками сравнивалось бы заодно всё,
    // что успело измениться в ядре между ними.
    //
    // И вторая причина, которой у вытаптывания нет: наблюдатель обязан
    // рисовать решение ТОГО ЖЕ разума, каким его принял зверь. Живи выбор
    // разума в сборке сервера — панель показывала бы дорогу, по которой
    // никто не идёт.
    bool lotteryMind = false;
};
