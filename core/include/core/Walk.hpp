...
//
// Складывается шаг из шести слагаемых, и все они — очки, а не запреты:
...
//   6. Встревоженная земля (необязательная, вес dangerWeight): клетка,
//      где недавно рвали зубами, теряет в цене. Не желание и не страх —
//      это ниже желаний, в самом выборе ноги, и весит для каждого
//      животного своё (травоядное уже уходит от крови через страх, п.10a,
//      и получает здесь вес 0, чтобы не считать одно и то же дважды).
...
//
// danger(x, y) — шестое слагаемое очков, необязательное по смыслу:
// встревоженность клетки (DangerComponent), с весом dangerWeight. Сам
// Walk.hpp не знает ни про хищников, ни про травоядных — только про
// клетки и память; кому и сколько стоит кровь под ногами, решает
// вызывающая сторона через dangerWeight (0 отключает слагаемое целиком).
template <typename Standable, typename Danger>
WalkStep chooseStep(MovementComponent& memory, int x, int y, int aimDirection, Standable&& standable,
                    Danger&& danger, int dangerWeight, std::uint64_t& random) {
    ...
    for (int dir = 0; dir < 8; ++dir) {
        ...
        if (dangerWeight != 0) {
            score -= dangerWeight * danger(nx, ny) / kFull;
        }
