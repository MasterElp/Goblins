...
// Куда существо может поставить ногу: не на занятый непроходимым объектом
// тайл, не мимо Области, не в воду и не выше того, куда оно вообще
// забирается.
//
// Вода — стена для всех: животное не плавает и брода не знает, поэтому
// клетка, где вода есть вообще, непроходима так же, как булыжник. Река делит
// карту, а не замедляет ход. Руки здесь не помогают: за воду не ухватишься.
//
// who — чем существо берёт высоту (core/Climb.hpp): kOnLegs у всякого, кто
// ходит на четырёх, kOnHands у того, у кого руки. Приходит снаружи, потому
// что закон про существ ничего не знает.
//
// Проверка высоты стоит ЗДЕСЬ, а не в решении каждого существа, и по той же
// причине, по какой в самом законе стоит запрет строить под деревом: закон,
// мимо которого можно пройти одной забытой проверкой, рано или поздно
// обойдут. Спрашивают его четверо — обе системы, наблюдатель и волна дороги.
//
// Вопрос остаётся об ОДНОЙ клетке, и это не мелочь оформления: высота —
// свойство места, а не перехода. Была попытка сделать преградой уступ, то
// есть разницу высот между двумя клетками; она потребовала бы спрашивать
// закон о паре клеток, а мир — резать на полки. Полки запирали зверьё каждое
// на своей, и от них отказались (см. core/Climb.hpp).
//
