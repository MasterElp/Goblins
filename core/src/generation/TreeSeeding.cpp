...
bool plantTree(World& world, int x, int y, const PlantGenomeComponent& archetype, float mutationRate, int pace,
               std::uint64_t& state) {
    ...
    plant.age = static_cast<int>(
        randomBelow(state, static_cast<std::uint64_t>(std::max(1, plantMaxAgeOf(genome, pace)))));
...
void seedTrees(World& world, const PlantParams& params, unsigned seed) {
    ...
    for (const auto& archetype : species) {
        ...
        seedNest(
            ...
            [&](int x, int y) { return plantTree(world, x, y, archetype, mutationRate, params.treePace, state); },
            state);
    }
}
