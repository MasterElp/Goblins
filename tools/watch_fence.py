"""Строится ли забор — и есть ли ради чего.

Мера двойная нарочно. Забор гоблины ставят не вообще, а ОТ ЗУБОВ (см.
core/Bound.hpp и третью причину строить в GoblinSystem), поэтому число кольев
само по себе не значит ничего: в мире без хищников ноль заборов — правильный
ответ, а не поломка. Здесь поэтому рядом идут две дорожки, поголовье хищников
и прочность плетня, и читать их надо вместе.
...
K_LIVED_IN_WIRE = 50      # kLivedIn = 500, на проводе сотые
