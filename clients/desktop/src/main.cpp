...
int main(int argc, char** argv) {
    ...
    // мир продолжает жить, просто его состояние сюда не едет. Причин
    // четыре, и все четыре сходятся в одно решение — свёрнутое окно
    // (смотреть некуда), окно без фокуса (смотрят в другое), клавиша
    // (смотреть не хочу) и открытый не тот экран (смотреть нечем: в меню,
    // в настройках и в выборе мира от мира на экране нет ничего). Место
    // здесь, а не на экране мира: свернуть окно можно с любого экрана, и
    // решение одно на всё приложение.
    //
    // Дороже всего тут не трафик, а сборка снимка на сервере: он собирает
    // и сличает всю карту десять раз в секунду, пока считает, что на него
    // смотрят, — и делает это даже на паузе, когда сличать нечего. Пока
    // не смотрит НИКТО, сервер не делает её вовсе.
    ...
    while (screen != AppScreen::Exit) {
        ...
        // Экран мира здесь один — и наблюдение, и подбор параметров
        // генерации (AppScreen.hpp), — поэтому условие на экран одно.
        network.sendUpdatesEnabled(screen == AppScreen::World && watchByChoice && !IsWindowMinimized() &&
                                    IsWindowFocused());
