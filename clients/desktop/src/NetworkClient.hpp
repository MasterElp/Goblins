...
struct WorldState {
    ...
    struct Watched {
        ...
        // Кого наблюдаемый гоблин знает: имя в мире, тепло 0..100 и — если
        // знакомый ещё жив — где он сейчас стоит.
        //
        // Место приезжает вместе с теплом, а не берётся клиентом из списка
        // гоблинов, по той же причине, по какой оттуда не берётся память:
        // сервер отвечает на вопрос "кого он знает" целиком, одним ответом, и
        // склеивать его половинки на клиенте значило бы завести здесь второй
        // закон о том же. placed = false — знакомого в мире больше нет
        // (умер), но он ещё не забыт: связь остывает сама, и вычёркивать её
        // некому.
        struct Face {
            std::uint64_t id = 0;
            int warmth = 0;
