...
struct Layers {
    ...
    bool animals = true;
    // Встревоженность земли — свой выключатель, а не часть слоя животных:
    // это состояние почвы, и смотрят на него отдельно — когда разбираются,
    // почему стадо ушло с этого края карты.
    bool danger = true;
};
