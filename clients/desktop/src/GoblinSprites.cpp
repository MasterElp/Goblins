...
Pose poseOf(const std::string& desire, bool walking, bool loaded) {
    ...
    // Сюда попадают "idle", "mate", "talk" и всякое желание, которого в мире
    // ещё не было, когда рисовались кадры. Стояние — честный ответ на все:
    // ничем другим они со стороны и не выглядят, а разговор — тем более. Двое
    // стоящих вплотную и есть весь его вид, и отдельный кадр прибавил бы к
    // этому только размахивание руками.
    return Pose::Stand;
}
