...
Pose poseOf(const std::string& desire, bool walking, bool loaded) {
    ...
    // Сюда попадают "idle", "mate", "talk", "flee" и всякое желание, которого
    // в мире ещё не было, когда рисовались кадры. Стояние — честный ответ на
    // все: ничем другим они со стороны и не выглядят, а разговор — тем более.
    // Двое стоящих вплотную и есть весь его вид, и отдельный кадр прибавил бы
    // к этому только размахивание руками.
    //
    // Спасающийся сюда доходит редко: он почти всегда идёт, и его рисует
    // ходьба выше. Стоит он ровно в одном случае — когда зубы уже в соседней
    // клетке и он отмахивается; своего кадра для этого нет, и заводить его
    // стоит тогда, когда станет видно, что этот случай на карте частый.
    return Pose::Stand;
}
