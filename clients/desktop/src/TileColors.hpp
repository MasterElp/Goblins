...
// Постройки. Навес — тёмная тень поверх клетки (он над головой и закрывает
// собой всё, что под ним), подстилка — светлое пятно (она под ногами и её
// видно рядом). Оба тем заметнее, чем прочнее: полунавес и выглядит наполовину.
//
// Различать их с одного взгляда обязательно: сердце лагеря — это клетка, где
// сошлись куча, навес и подстилка, и увидеть её надо мгновенно.
inline Color canopy(Color base, float condition) {
    static const Color roof{62, 54, 44, 255};
    return lerp(base, roof, 0.25f + 0.5f * std::clamp(condition, 0.0f, 1.0f));
}

inline Color bedding(Color base, float condition) {
    static const Color mat{198, 186, 138, 255};
    return lerp(base, mat, 0.2f + 0.45f * std::clamp(condition, 0.0f, 1.0f));
}

// Забор — тёмная жердь поперёк клетки. Цвета коры, как и жерди навеса: его
// из тех же веток и сплели. Темнее подстилки и холоднее навеса — он не
// укрытие, а преграда, и спутать его с крышей нельзя: под навесом спят, а
// через забор не проходят.
inline Color fence(Color base, float condition) {
    static const Color wattle{88, 66, 44, 255};
    return lerp(base, wattle, 0.3f + 0.55f * std::clamp(condition, 0.0f, 1.0f));
}

// Стройплощадка — замысел, а не вещь: показывается редкой холодной крапиной,
// чтобы её было видно, но чтобы она не спорила с построенным. Задумано —
// ещё не сделано.
inline Color site(Color base) {
    static const Color mark{120, 150, 170, 255};
    return lerp(base, mark, 0.35f);
}
