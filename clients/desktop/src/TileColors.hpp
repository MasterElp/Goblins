...
// намеренно тёмная, густая, с уходом в коричневое: роща на карте должна
// читаться пятном с первого взгляда, иначе весь смысл островков теряется
// — а различать деревья по оттенкам зелени, среди зелени же, нельзя.
// Пять — верхний предел числа видов деревьев (core::kMaxTreeSpecies).
inline Color treeSpecies(int species) {
    static const Color palette[5] = {
        {36, 78, 42, 255}, {58, 70, 32, 255}, {28, 62, 58, 255}, {72, 84, 38, 255}, {44, 58, 30, 255},
    };
    if (species < 0) {
        return palette[0];
    }
...
// Дерево поверх почвы. В отличие от травы, цвет вида подмешивается почти
// целиком даже у подроста: дерево занимает клетку и стоит на ней десятки
// тысяч тиков, и видеть его надо всегда, а не только когда оно выросло.
// Развитость при этом всё же читается — по густоте.
inline Color tree(Color soilColor, int species, float growth) {
    return lerp(soilColor, treeSpecies(species), 0.65f + 0.3f * std::clamp(growth, 0.0f, 1.0f));
}
