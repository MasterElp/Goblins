...
//      "bush_species": [{"species": N, <черты генома>}, ...]
...
//                          "predators": [...]},
//      "animals": [{"id", "x", "y", "species", "kind", "sex", "adult_size",
//                    "desire", "growth", "health"}, ...]
...
//                    "growth", "health", "fatigue", "carried", "material"}, ...]
...
//                 "growth", "seeds", "trees", "bushes", "berries",
//                 "trampled", "store", "canopy", "bedding", "site",
//                 "site_material"}}
...
//              "canopy"/"bedding" — прочность построек в сотых, 0 — нет
//              постройки; "site" — вид задуманного на клетке (0 нет, 1 навес,
//              2 подстилка); "site_material" — материал, лежащий В КУЧЕ на
//              этой клетке, приведённый к одной мерке (materialIn,
//              core/Build.hpp), величина СЧЁТНАЯ и потому не округляемая.
//              Стройка теперь берёт материал из кучи под ногами и из рук
//              работающего, поэтому слой и переехал с площадки на кучу: по
//              нему видно, стоит стройка без материала или идёт.
//              "store" — сколько ВСЕГО лежит в куче на тайле: еда, солома,
//              ветки и то, что появится потом, одним числом (core/Resources.hpp).
//              Величина счётная и не округляется, как минералы и перегной;
//              ноль значит "кучи нет". Роспись по видам на карту не идёт —
//              плотный массив на каждый ресурс стоил бы всей карты в каждой
//              дельте; она едет только для клетки, за которой следят, в
//              "watched".
//              "trampled" — утоптанность земли в сотых, как влажность:
//              её набивают ноги и она зарастает сама (core/Trample.hpp),
//              поэтому слой идёт и в дельты, в отличие от каменистости.
//              "bushes" — вид стоящего в клетке куста или -1, третий слой
//              родов растения; "berries" — сколько ягод на нём висит,
//              величина СЧЁТНАЯ и потому не округляемая, как минералы и
//              перегной. Ноль ягод — не пустая клетка: куст стоит, просто
//              обобран.
...
//      плюс "fatigue", "carried" и "material": [индекс, значение, ...] —
//      поля, которых у зверя нет: усталость, еда в руках и материал в них же.
...
//       {"kind": "goblin", "id": N, "x", "y", "tribe": N, "sex": "...",
//        "desire": "...", "doing": "...", "groups": [...],
//        "rest": [x, y, годность, ...],  -- пригодность округи для отдыха,
//                сотыми, тем же законом, каким её меряет сам гоблин
//                (core/Rest.hpp)
//        "knows": [{"x", "y", "kind": "food"|"water"|"rest"|"work",
//                   "strength": 0..100}, ...],  -- что он помнит
//        "faces": [{"id": N, "warmth": 0..100, "x", "y"}, ...]}  -- кого он
//                знает. "x"/"y" есть, только если знакомый ещё в мире: связь
//                переживает того, с кем она была, и остывает сама
//                (core/Bonds.hpp) — вычёркивать умерших некому.
//        Нрав и склонности приезжают группами "Character" и "Interests" —
//                там же, где тело и геном. Ни нрава, ни связей нет ни в
//                карточке, ни в дельте, и это не упущение: нрав за жизнь не
//                меняется, а восемь знакомств на каждого гоблина каждый тик
//                весили бы больше всего остального провода. Нужны они по
//                одному — у того, за кем следят, — ровно как память места
//        "doing" — чем он занят прямо сейчас, ГОТОВОЙ строкой ("building the
//                canopy", "carrying material to the site", "drinking"). В мире
//                этого не записано: желание есть в компоненте, а что из него
//                вышло в прошедшем тике — нигде, и заводить ради показа поле
//                состояния мира было бы платой не по товару. Сервер
//                собирает строку из фактов на клетке под ногами — тех же, по
//                которым выбирает действие GoblinSystem, и в том же порядке.
//                Клиент печатает её так же, как печатает имя желания, и не
//                знает, какие занятия бывают
...
private:
    ...
    struct LayerSnapshot {
        ...
        // Кусты: вид куста, стоящего в клетке, или -1. Третий слой по той же
        // причине, что и второй, — своя нумерация видов (kBushTraits).
        std::vector<int> bushes;
        // Ягоды, висящие на кусте: счётные штуки, а не доля, поэтому
        // округления показа к ним не применяется (как к минералам и
        // перегною). Ноль значит "ягод нет"; куст при этом стоит — обобранный
        // ягодник виден по слою bushes, и это разные вещи.
        std::vector<int> berries;
        // Утоптанность, наоборот, меняется каждый тик — её набивают ноги и
        // сама она зарастает (core/Trample.hpp), — поэтому идёт и в дельты.
        std::vector<int> trampled;
        // Куча принесённой еды: счётная величина, как минералы и перегной, и
        // потому без округления показа. Меняется каждый тик — её приносят,
        // едят и она гниёт, — значит идёт и в дельты.
        std::vector<int> store;
        // Постройки: прочность навеса и подстилки в сотых (core/Build.hpp).
        // Ноль значит "нет". Меняются каждый тик — их строят и они ветшают,
        // — значит идут и в дельты.
        std::vector<int> canopy;
        std::vector<int> fence;
        std::vector<int> bedding;
        // Стройплощадка: вид задуманного (0 — нет, 1 — навес, 2 — подстилка).
        // Замысел виден всякому — на то он и общий.
        std::vector<int> site;
        // Материал, лежащий на площадке, приведённый к одной мерке
        // (materialStrength, core/Build.hpp): солома и ветки одним числом.
        // Одним, а не двумя слоями, потому что работа тратит именно силу
        // материала и в неё оба и складываются; чем набрано, видно в
        // карточке того, кто на площадке стоит.
        //
        // Без этого числа стоящая стройка выглядит точно так же, как идущая,
        // — а разница между ними ровно в нём: работать нечем.
        ...
        struct AnimalView {
            ...
            bool predator = false;
            Sex sex = Sex::Female;
            // Размер взрослого, в тысячных нынешнего зверя (adult_size,
            // core/Body.hpp). Черта генома: за жизнь не меняется — значит,
            // только карточка, без дельты (CLAUDE.md, круг нового поля).
            //
            // Клиенту он нужен, чтобы нарисовать зверя его величины: рядом с
            // growth это и даёт настоящий размер тела. Без него десять лосей
            // и сорок зайцев выглядят одинаковыми точками, ради чего всё и
            // затевалось.
            int adultSize = 0;
        };
        ...
        struct GoblinView {
            ...
            // Усталость, сотые доли. Меняется за жизнь — значит, едет и в
            // карточке, и в дельте (CLAUDE.md, круг нового поля существа).
            int fatigue = 0;
            // Еда в руках, в сотых биомассы. Тоже меняется за жизнь и тоже
            // едет обоими путями: без неё по карточке нельзя понять, почему
            // гоблин идёт домой мимо ягодника.
            int carried = 0;
            // Материал в руках, в сотых биомассы: трава и ветки одним числом.
            // Порознь они нужны только на площадке (там ветка стоит трёх
            // соломин), а в карточке важно другое — несёт он что-нибудь или
            // идёт налегке.
            int material = 0;
            // Не меняется за всю жизнь и потому едет только в карточке.
            int tribe = 0;
