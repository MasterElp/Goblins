...
//              2 подстилка); "site_material" — материал, лежащий на площадке,
//              приведённый к одной мерке (materialStrength, core/Build.hpp),
//              величина СЧЁТНАЯ и потому не округляемая. Ноль на площадке
//              значит "работать нечем" — по нему и отличается стоящая
//              стройка от идущей. Все четверо меняются за жизнь мира и идут
//              в дельты.
//              "store" — еда в куче, принесённой на тайл чьими-то руками
//              (core/Store.hpp). Величина счётная и не округляется, как
//              минералы и перегной; ноль значит "кучи нет".
