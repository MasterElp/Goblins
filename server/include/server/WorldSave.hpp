...
//
// Порядок ключей в файле именно такой, как записано ниже, и это не
// украшение: заголовок ("format", "version", "info") идёт первым, а
// "entities" — последним. Список миров в меню читает только заголовок, и
// заголовок впереди позволяет ему прочесть первые двести байт вместо
// сорока мегабайт (см. readSaveHeader в WorldSave.cpp). Файлы, записанные
// раньше, лежат в алфавитном порядке nlohmann — их заголовок ищется в
// хвосте, а разбор от этого не зависит вовсе.
//
...
//                   "bush_species": [ {"species": 0, "<черта>": V, ...}, ... ],
...
//
// "bush" (BushComponent) — такой же тег и по тому же образцу: род растения
// решает, по какой таблице читается геном ("bush_species", kBushTraits) и
// чем растение станет при загрузке. Род спрашивается одной функцией на весь
// проект (core::plantKindOf, core/PlantKind.hpp) — файл мира, сетевой слой и
// снимок клеток обязаны отвечать на этот вопрос одинаково.
//
// "berries" (BerryComponent) — ягоды на кусте и ушедшие в них крупицы
// минералов. Пишутся отдельно от "plant", потому что и живут отдельно: ягоды
// есть только у куста. Крупицы обязаны сохраниться вместе с ягодами —
// вещество в этом мире не появляется и не пропадает, а без них загруженный
// ягодник вернул бы в землю меньше, чем взял.
//
...
//
// Читается только заголовок, а не файл целиком: состояние мира тут не
// нужно, и разбирать ради семи полей десятки мегабайт незачем. Уже
// прочитанные заголовки держатся в памяти процесса до тех пор, пока файл
// не изменится (по размеру и времени записи) — список рассылается не
// только по запросу меню, но и при каждом подключении клиента, после
