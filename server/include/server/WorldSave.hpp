...
//                   "store": {"<ресурс>": N, ..., "minerals": N},
//                   "building": {"canopy","bedding"},
//                   "site": {"kind"},
//                   "carried": {"<ресурс>": N, ..., "minerals": N},
...
//
// "store" (StoreComponent) — куча принесённой еды, состояние земли рядом с
// перегноем и падалью. Потерять её нельзя тем более, что вырасти заново она
// не может: всё, что в ней лежит, кто-то принёс руками. "carried"
// (CarriedComponent) — то, что несут в этих руках прямо сейчас; у существа с
...
//
// "store" и "carried" пишутся ПО ИМЕНАМ ВИДОВ ресурсов
// (core::ResourceKind): "food", "straw", "twigs" и то, что появится потом.
// Новый вид попадает в файл сам, а старый файл, где его нет, читается нулём —
// тот же приём, что у черт генома, и по той же причине: список ведёт формат,
// а не наоборот. Нулевые виды не пишутся вовсе, чтобы не возить пустоту.
//
// "building" (BuildingComponent) — прочность навеса и подстилки, "site"
// (SiteComponent) — задуманное на клетке, и только оно: материал лежит в куче
// на той же клетке, а неделящегося остатка труда не бывает вовсе (цены
// построек выбраны так, чтобы прочность росла целыми числами). И то, и другое потерять нельзя тем более,
